...
def process_video_frames(video_source_path, fps):
  imagestore = []
  # Extract the last directory name from the source path
  output_name = os.path.basename(os.path.normpath(video_source_path))
  ...
  cnt = 0
  for video in os.listdir(video_source_path):
  ...
  savedir = os.path.join("npys", f"{output_name}.npy")
  print("Saving images in:", savedir)
  np.save(savedir, imagestore)
...
if __name__ == "__main__":
  ...
  process_video_frames(args.source_vid_path, args.fps)
